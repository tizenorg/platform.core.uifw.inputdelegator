...
#define CREDENTIAL_GEARINPUT "{ \"installation type\": \"Preinstalled\", \"credentials\": [\"Samsung_TizenGear_KeyboardSSLHTTPV2_20141106\", \"4b555aa86bbd2248d715635d40eae32bdd11b1cecc7eb168dc558047ab2df48ebba9485b39988cd37c6785127760dcb6e0f6e0448e5ccdcb621e36f0d0d4ac70\", \"https://ssvnv2-ncs-eng-usa.nuancemobility.net:443/NmspServlet/\"], \"intermediate mode\": \"NoUtteranceDetectionWithPartialRecognition\" }"
...
//	stt_is_samsung_asr(&bval);
	...
	if( bval == true && !language.compare("en_GB")) {
		...
		//ret = stt_start_ex(handle, "en_US", asrtype.c_str(), CREDENTIAL_GEARINPUT);
        ret = stt_start(handle, "en_US", asrtype.c_str());
	} else {
		//ret = stt_start_ex(handle, language.c_str(), asrtype.c_str(), CREDENTIAL_GEARINPUT);
