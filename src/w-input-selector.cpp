...
void init_changeable_theme(void)
{
/*
	string shared_res_path = get_shared_resource_path();

	string changeable_color_path = shared_res_path + "org.tizen.inputdelegator_color.xml";
	string changeable_font_path = shared_res_path + "org.tizen.inputdelegator_font.xml";

	ea_theme_changeable_ui_enabled_set(EINA_TRUE);

	Ea_Theme_Color_Table *table;
	table = ea_theme_color_table_new(changeable_color_path.c_str());

	Ea_Theme_Font_Table *font_table;
	font_table = ea_theme_font_table_new(changeable_font_path.c_str());
...
//	stt_is_samsung_asr(&bVal);
	...
	if(bVal){
		...
		//Nuance
		if(is_tos_Nuance_agreed() == EINA_FALSE){
			if( is_sap_connection() == EINA_TRUE){
				ise_show_tos_popup(ad);
			} else {
				launch_bt_connection_popup(ad);
			}
		} else {
...
void _app_service(app_control_h service, void* user_data)
{
	...
	ret = app_control_get_extra_data(service, "launch_type", &context);
	if (ret == APP_CONTROL_ERROR_NONE) {
		if(!strcmp(context, "voice_input")) {
			app_data->app_type = APP_TYPE_STT;
			_stt_clicked_cb( (void *)app_data, NULL, NULL );
			goto ACTIVATE;
		} else if(!strcmp(context, "emoticon_input")) {
			app_data->app_type = APP_TYPE_EMOTICON;
			_emoticon_clicked_cb( (void *)app_data, NULL, NULL);
			goto ACTIVATE;
        } else if(!strcmp(context, "keyboard_input")) {
			...
			_keyboard_clicked_cb( (void *)app_data, NULL, NULL);
			goto ACTIVATE;
		} else if(!strcmp(context, "reply_input")) {
