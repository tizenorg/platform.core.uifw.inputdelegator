...
{
    int ret = vconf_set_bool( VCONFKEY_USER_AGREED_TOS, bSet );
    if ( ret != VCONF_OK )
    {
...
Eina_Bool is_tos_Nuance_agreed()
{
    ...
    int bVal = 0;
    int ret = vconf_get_bool(VCONFKEY_USER_AGREED_TOS, &bVal );
    if ( ret != VCONF_OK )
    {
        PRINTFUNC(DLOG_ERROR,"vconf_get_bool failed ... [%d]!!!", ret);
        return EINA_FALSE;
    }
...
static void _response_cb(void *data, Evas_Object *obj, void *event_info)
{
    ...
    set_tos_Nuance_agreed(EINA_TRUE);
